...
if (!defined('_DESIGNER_FUNCTIONS')) {
    ...
    $themeHelper->themeImagesPath = JURI::root(true) . '/templates/' . JFactory::getApplication()->getTemplate() . '/images/';

    $pageDesc = $document->getDescription() ? $document->getDescription() : $config->get('sitename');

    $seoTags = array();
    $seoTags['canonical'] = array('tag' => 'link', 'rel' => 'canonical', 'href' => JUri::getInstance()->toString());
    ...
    function getProductsScript() {
        $themeName = JFactory::getApplication()->getTemplate();
        $url = JUri::root() . 'index.php?option=com_ajax&format=json&template=' . $themeName . '&method=siteproducts';
        ...
        foreach ($children as $child) {
            ...
            if ($isVm) {
                ...
                if (property_exists($child, 'link')) {
                    ...
                    $href = JRoute::_(ContentHelperRoute::getCategoryRoute($child->id));
                }
